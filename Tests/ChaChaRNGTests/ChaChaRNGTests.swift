...
final class ChaChaRNGTests: XCTestCase {
    func testChaCha8RNG() {
        var rng = ChaCha8RNG(seed: .zero)
        
        for _ in 0..<3141 {
            let _ = rng.next() as UInt64
        }
        
        XCTAssertEqual(.random(in: 0...UInt64.max, using: &rng), [
    ...
    func testNextUInt8() {
        ...
        XCTAssertEqual((0..<128).map({ _ in rng.next() }), [
    ...
    func testNextUInt16() {
        var rng = ChaCha20RNG(seed: .zero)
        XCTAssertEqual((0..<64).map({ _ in rng.next() }), [
    ...
    func testNextUInt32() {
        var rng = ChaCha20RNG(seed: .zero)
        XCTAssertEqual((0..<32).map({ _ in rng.next() }), [
    ...
    func testNextUInt64() {
        var rng = ChaCha20RNG(seed: .zero)
        XCTAssertEqual((0..<16).map({ _ in rng.next() }), [
    ...
    func testStreamAndFill() {
        var rng = ChaCha20RNG(seed: .zero, stream: 0xb61e6e6a48c285)
